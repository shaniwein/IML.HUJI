...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.include_intercept_:
            # Insert col of ones:
            obj = 0 # Defines the index before values inserted.
            X = np.insert(X, obj, values=1, axis=1)
        self.coefs_ = (inv((np.transpose(X) @ X) + (self.lam_ * np.identity(X.shape[1])))) @ (np.transpose(X) @ y) 
